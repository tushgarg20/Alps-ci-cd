...
#Arijit: Added hack for GT0
$formulas->{"FPU2_Utilization"}			= &CalcFPU2_Util();
$formulas->{"FPU0_Utilization"}			= &CalcFPU0_Util();
...
sub InitSimDataStructure {
   ...
   push (@$lrSimData, {
	 ...
	 #Arijit: Added hack for GT0 
         ...
         'StatObj' => MultiStatIStatObj->new({'m_StatRegex' => 'uCore_\\d+\\.SIMD4_Executed_(0|2)',
                                              'm_ReportDataAs' => 'SUM',
                                              'm_DoTestMaxVal' => 0})
   });
   ...
   push (@$lrSimData, {
	 ...
	 #Arijit: Added hack for GT0 
         #'StatObj' => MultiStatIStatObj->new({'m_StatRegex' => 'uCore_\\d+\\.SIMD4_Executed_0',
         'StatObj' => MultiStatIStatObj->new({'m_StatRegex' => 'uCore_\\d+\\.SIMD4_Executed_2',
                                              'm_ReportDataAs' => 'AVERAGEDIV1',
                                              'm_DoTestMaxVal' => 0})
   });
   ...
   foreach my $d (@eu_pln_opcodes)
   {
	   ...
	   push (@$lrSimData, {
			 ...
			 'StatObj' => MultiStatIStatObj->new({'m_StatRegex' => 'uCore_\\d+\\.Pipe_SIMD4_(0|2)_' . $media_int32 . '_' . $d . '$',
												  'm_ReportDataAs' => 'SUM',
												  'm_DoTestMaxVal' => 0})
	   });
	   ...
	   push (@$lrSimData, {
			 ...
			 #Arijit: Added hack for GT0 
			 #'StatObj' => MultiStatIStatObj->new({'m_StatRegex' => 'uCore_\\d+\\.Pipe_SIMD4_0_INT_(16|8)_' . $d . '$',
			 'StatObj' => MultiStatIStatObj->new({'m_StatRegex' => 'uCore_\\d+\\.Pipe_SIMD4_(0|2)_INT_(16|8)_' . $d . '$',
												  'm_ReportDataAs' => 'SUM',
												  'm_DoTestMaxVal' => 0})
	   });
	   ...
	   push (@$lrSimData, {
			 ...
			 'StatObj' => MultiStatIStatObj->new({'m_StatRegex' => 'uCore_\\d+\\.Pipe_SIMD4_1_INT_(16|8)_' . $d . '$',
												  'm_ReportDataAs' => 'SUM',
												  'm_DoTestMaxVal' => 0})
	   });
   }
	...
	for(my $i = 0 ; $i < $num_rows ; $i++)
	{
		for(my $j = 0 ; $j < $num_eu ; $j++)
		{
			push (@$ToggleSimData, {
			 ...
			 'StatObj' => MultiStatIStatObj->new({'m_StatRegex' => 'uCore_' . $i . $j . '\.m_GRF_SIMD4_(0|2)_toggles_\d' ,
												  'm_ReportDataAs' => 'MAX',
												  'm_DoTestMaxVal' => 0})
			});
...
#Arijit: Added hack for GT0
sub CalcFPU2_Util{
	return $base_stat_tbl->{"FPU2BusyClks"}/$base_stat_tbl->{"GennysimStatClks"} ;
} ;

sub CalcFPU0_Util{
	return $base_stat_tbl->{"FPU0BusyClks"}/$base_stat_tbl->{"GennysimStatClks"} ;
} ;
...
#Arijit: Added hack for GT0
sub CalcEU_Active{
	my $blah1 = $formulas->{"FPU0_Utilization"};
	my $blah2 = $formulas->{"FPU2_Utilization"};
	my $blah3 = $formulas->{"EM_Utilization"};
	print "FPU0 $blah1 FPU2 $blah2 EM $blah3\n";
	if(($formulas->{"FPU0_Utilization"} > $formulas->{"EM_Utilization"}) && ($formulas->{"FPU2_Utilization"} > $formulas->{"EM_Utilization"}))
	{
		if ( $formulas->{"FPU0_Utilization"} > $formulas->{"FPU2_Utilization"} )
		{
			return  $formulas->{"FPU0_Utilization"} ;
		}
		return  $formulas->{"FPU2_Utilization"} ;
	}
	if(($formulas->{"FPU0_Utilization"} > $formulas->{"EM_Utilization"}) && ($formulas->{"FPU2_Utilization"} < $formulas->{"EM_Utilization"}))
	{
		return  $formulas->{"FPU0_Utilization"} ;
