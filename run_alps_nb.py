...
def is_float(value):
    try:
        ...
        return False
try:
    if( (options.voltage and is_float(options.voltage) == False ) or (options.sf_votage and is_float(options.sf_voltage) == False) 
...
if not options.run_local:
    ...
    build_alps_cmd = ['python', '/p/gat/tools/gsim_alps/build_alps.py', '-i', input_file, '-r', res, '-a', options.dest_config, '-o', yaml ]
    if(options.tg_file):
    ...
    input_file = options.user_dir + '/' + "input_files/" + cfg_data['ALPS Input File'][0]
    build_alps_cmd = ['python', build_alps_script, '-i', input_file, '-r', res, '-a', options.dest_config, '-o', yaml, '-m', options.method, '--ecw' ]
    if(options.tg_file):
        build_alps_cmd += ['-t', '%s/%s' % (options.output_dir, options.tg_file), '-z', '%s/alps_Timegraph.txt' % options.output_dir]
    if(options.run_debug):
        build_alps_cmd += ['--debug']
